...
import { Jasa, Pesanan, User, Pendapatan } from '../types';
...
export function RiwayatPendapatan({ onNavigate, activeUser, refreshTrigger }: RiwayatPendapatanProps) {
  ...
  const pendingCair = revenues.filter(r => !r.statusCair).reduce((sum, item) => sum + item.amount, 0);
  const successCair = revenues.filter(r => r.statusCair).reduce((sum, item) => sum + item.amount, 0);

  const handleWithdrawSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (pendingCair === 0) {
      alert('Maaf, saldo dompet Anda kosong.');
      return;
    }
    ...
    const desc = `Pencairan dana sebesar ${formatRupiah(pendingCair)} via ${methodLabel} (${withdrawAccountNo} a/n ${withdrawAccountName})`;

    // update all matching to true status
    const all = KampusFixDB.getPendapatan();
    const updated = all.map(r => {
      if (r.penyediaId === activeUser.id && !r.statusCair) {
        return { 
          ...r, 
          statusCair: true,
          withdrawNote: desc,
          withdrawMethod: methodLabel,
          withdrawAccount: withdrawAccountNo,
          withdrawRequestedName: withdrawAccountName,
    ...
    KampusFixDB.savePendapatan(updated);
    
    // Also call single-item sync write through to Firestore for each modified pendapatan item
    updated.forEach(item => {
      if (item.penyediaId === activeUser.id && item.statusCair) {
        KampusFixDB.syncSingleItem('pendapatan', item);
      }
    });

    setRevenues(updated.filter(r => r.penyediaId === activeUser.id));
    setSuccess(true);
