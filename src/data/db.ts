...
import { User, Jasa, Pesanan, Pendapatan, Notifikasi, Review, UserRole, StatusPesanan } from '../types';
import { 
  collection, 
  onSnapshot, 
  doc, 
...
export class KampusFixDB {
  ...
  static writeAndSync<T>(key: string, val: T): void {
    ...
    if (Array.isArray(val)) {
      this.syncArrayToFirestore(key, val);
    }
  }
  ...
  static updatePesananStatus(id: string, newStatus: StatusPesanan, note?: string): Pesanan {
    ...
    if (newStatus === 'menunggu_pembayaran') {
      ...
      this.addNotification(ord.penyediaId, 'Pesanan Selesai!', `Pencari mengonfirmasi pekerjaan selesai. Dana Rp ${ord.jasaPrice.toLocaleString('id-ID')} dicairkan.`, 'pesanan', ord.id);
...
export class FirebaseSync {
  ...
  static setupListener(key: string, fallbackDefaults: any[]) {
    ...
    onSnapshot(collection(db, firestoreCollection), (snapshot) => {
      ...
      console.error(`Firebase synchronizer listener for collection ${firestoreCollection} failed:`, error);
